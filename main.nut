 ...
 */
class EvoAI extends AIController
{
/* private: */
...
/* public: */
	 ...
	 * @note This calls EvoAI::ScanForDepot first, so there is no need to
	 *   do that explicitly.
	 */
...
function EvoAI::BuildHQ(station_id, width, height)
{
...
function EvoAI::GetStationManager(station_id)
{
...
function EvoAI::Load(version, data)
{
...
function EvoAI::CallLoad()
{
	if (this._save_data == null) return;

	AILog.Info("Loading savegame saved with EvoAI " + this._save_version);
...
/* static */ function EvoAI::ScanForDepot(roadtile)
{
...
/* static */ function EvoAI::BuildDepot(roadtile)
{
	local depot = EvoAI.ScanForDepot(roadtile);
...
function EvoAI::SendVehicleToSellToDepot()
{
...
function EvoAI::TransportCargo(cargo, ind)
{
...
/* static */ function EvoAI::SomeVehicleTypeAvailable()
{
	return EvoAI.UseVehicleType("planes") || EvoAI.UseVehicleType("trains") ||
		EvoAI.UseVehicleType("trucks") || EvoAI.UseVehicleType("busses");
	/* TODO: add ships here as soon as support for them is implemented. */
}

function EvoAI::DoMaintenance()
{
	this.GetEvents();
	this.HandleEvents();
...
function EvoAI::SetCompanyName()
{
	...
	if (EvoAI.UseVehicleType("planes")) vehicle_types = vehicle_types | (1 << AIVehicle.VT_AIR);
	if (EvoAI.UseVehicleType("trains")) vehicle_types = vehicle_types | (1 << AIVehicle.VT_RAIL);
	if (EvoAI.UseVehicleType("trucks")) vehicle_types = vehicle_types | (1 << AIVehicle.VT_ROAD);
	if (EvoAI.UseVehicleType("busses")) vehicle_types = vehicle_types | (1 << AIVehicle.VT_ROAD);
	if (EvoAI.UseVehicleType("ships")) vehicle_types = vehicle_types | (1 << AIVehicle.VT_WATER);
...
function EvoAI::Start()
{
	...
	if (AICompany.GetName(AICompany.COMPANY_SELF).find("EvoAI") == null) {
		this.SetCompanyName();
	...
	this._save_data = null;
	AILog.Info("Loading done" + Parameters.A_MIN_PASS_SMALL);
	...
	while(1) {
		...
		if (!build_route && build_road_route > 0 && AIGameSettings.GetValue("vehicle.max_roadveh") * 0.9 > veh_list.Count()) {
			...
			// By commenting the next line out EvoAI will first build truck routes before it starts on bus routes.
